...
Copy-Item -LiteralPath $glassOutput -Destination (Join-Path $glassRoot 'dist/Codex-Glass-0.5.0-Portable-x64.exe')
if($Test) {
...
if($Package) {
    ...
    Compress-Archive -Path (Join-Path $glassPackage '*') -DestinationPath (Join-Path $glassRoot 'dist/Codex-Glass-0.5.0-Portable-x64.zip') -Force
}
